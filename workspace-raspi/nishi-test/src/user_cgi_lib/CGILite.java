...
public class CGILite {
	...
	public static boolean dump_f=false;
	private static boolean DEBG=false;
	private MyLog mlog;
	public String svr_lang="";	// add by nishi 2015.10.29
	...
	public void in(){
		...
		//SERVER_LANGUAGE add by nishi 2015.10.29
		...
		if(method.equals("GET")==true){
			...
			if(svr_lang.equals("C")==true){
				hin.setCgiUse(false); 	// set connection mode
			}
			else{
				hin.setCgiUse(true); 	// set CGI mode
			}
			if(dump_f){
				dump_din(hin);				
			}
	...
	private void get_req(){
		mlog.put(DEBG,"CGILite::get_req() :#1 start!");
		//QUERY_STRING
		if(env.get("QUERY_STRING")!= null){
			parm=env.get("QUERY_STRING");
			mlog.put(DEBG,"CGILite::get_req() :#2 parm="+parm);
			setParms();
		}
	}
	/*
	...
	private void post_req(HttpInputStreamCGI hin_r){
		...
		if(!multipart_f){
			...
			try {
				...
				mlog.put(DEBG,"CGILite::post_req() :#3 parm="+parm);
			}
			catch (IOException e) {
	...
	private void dump_din(HttpInputStreamCGI din_r){
		...
		try{
			...
			while (din_r.ready() == true && n < 10){
				...
				if(p_line.length()>0){
					System.out.println("CGILite::dump_din() :#1 p_line="+p_line+"\n");
				}
				else{
					System.out.println("CGILite::dump_din() :#2 p_line.length()="+p_line.length()+"\n");
					break;
				}
				n++;
			}
			...
			System.out.println("CGILite::read_din() :#90 error ="+e);
		}
	}		
	/*
	 * setParms()
	...
	public void setParms(){
		...
		//mlog.put(DEBG,"CGILite::setParms():#1 parmx.length="+parmx.length);
		for(int i=0;i< parmx.length;i++){
			String[] _parmx = parmx[i].split("=");
			if(_parmx.length>=2){
				//mlog.put(DEBG,"CGILite::setParms():#2 _parmx[0]="+_parmx[0]+",_parmx[1]="+_parmx[1]);
				if(parms.containsKey(_parmx[0])){
					String s=parms.get(_parmx[0])+0x00+_parmx[1];
					parms.put(_parmx[0], s);
					//mlog.put(DEBG,"CGILite::setParms():#3 passed!!");
				}
				else{
					parms.put(_parmx[0],_parmx[1]);
					//mlog.put(DEBG,"CGILite::setParms():#4 passed!!");
				}
			}
			else{
	...
	public void pr_as(String line){
		try {
			...
			System.out.println("CGILite::pr_as() :#1 DataOutputStream writeBytes() error="+e+"<br />");
		}
	}
	/*
	 * pr_kj(String line)
	...
	public void pr_kj(String line){
		try {
			...
			System.out.println("CGILite::pr_kj() :#1 DataOutputStream writeBytes() error="+e+"<br />");
		}
	}
	/*
	 * pr(String line)
	...
	public void pr(String line){
		try{
			...
			System.out.println("CGILite::pr() :#3 DataOutputStream writeBytes() error="+e+"<br />");
		}
	}
}
