...
//public class HttpInputStream extends DataInputStream {
	...
	public int _read(byte[] buf,int offs,int buf_len) throws IOException{
		...
		// CGI side read
		if(cgi_use){
			return super.read(buf,offs,buf_len);
		}
		...
		for(int _lct=lct;_lct>0;_lct--){
			...
			if(can_l > 0){
				...
				can_l-=r_lng;
				if(debug3)
					System.out.println(myClassName+"::_read() :#1 r_lng="+r_lng);
				break;
			}
			else{
				try {
		...
		if(debug)
			System.out.println(myClassName+"::_read() :#99 r_lng="+r_lng);
		return r_lng;
	}
	...
	public String readline() throws IOException{
		...
		lf=false;
		if(debug)
			System.out.println(myClassName+"::readline() :#1 ");
		while(lf==false){
			if(rem_lng < 2){
				read_off();
			}
		...
		if(debug){
			if(one_line.length()<3){
				System.out.println(myClassName+"::readline()() :#99 one_line="+one_line);
				System.out.println(myClassName+"::readline()() :#99.1 one_line="+util_lib.StringtoHex(one_line));
			}
		}
		return one_line;
	}
	...
	private void read_off() throws IOException{
		...
		if(debug)
			System.out.println(myClassName+"::read_off() :#99 rem_lng="+rem_lng);
	}
	/*
	...
	public int readData_notuse(byte[] out_f,int out_lng) throws IOException{
		...
		lf=false;
		if(debug2)
			System.out.println(myClassName+"::readData() :#1 cur_pos="+cur_pos+",buff_b[cur_pos]="+buff_b[cur_pos]+",rem_lng="+rem_lng);
		while(lf == false && out_lng > 0){
			if(rem_lng == 0){
				cur_pos=0;
	...
	private int readDataToLf(byte[] out_f,int pos,int out_lng){
		...
		if(debug2)
			System.out.println(myClassName+"::readDataToLf() :#1 cur_pos="+cur_pos+",buff_b[cur_pos]="+buff_b[cur_pos]+",rem_lng="+rem_lng);
		...
		if(debug2)
			System.out.println(myClassName+"::readDataToLf() :#99 cur_pos="+cur_pos+",rem_lng="+rem_lng+",out_l="+out_l);
		return out_l;
	}
	...
	public int readBlock(byte[] out_f,int out_lng) throws IOException{
		...
		if(debug2)
			System.out.println(myClassName+"::readBlock() :#1 cur_pos="+cur_pos+",buff_b[cur_pos]="+buff_b[cur_pos]+",rem_lng="+rem_lng);
		while(out_lng > 0){
			// buff_b remain data length == 0
			if(rem_lng == 0){
				if(debug2)
					System.out.println(myClassName+"::readBlock() :#2 pos="+pos+",out_lng="+out_lng);
				// read execute
			...
			// require read size >= buff_b remain data length
				...
				if(debug2)
					System.out.println(myClassName+"::readBlock() :#3 pos="+pos+",out_lng="+out_lng+",rem_lng="+rem_lng);
			...
			// require read size < buff_b remain data length
				...
				if(debug2)
					System.out.println(myClassName+"::readBlock() :#4 pos="+pos+",out_lng="+out_lng+",rem_lng="+rem_lng);
